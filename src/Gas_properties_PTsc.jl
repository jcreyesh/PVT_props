...
#---------------------------------------------------------------------------------------------------------------------------------
""" Correlación de Sutton - 1985 \n
PTsc\\_Sutton\\_Ge(Ge_gas) \n
\\- Argumentos - \n
\t Ge_gas -> Gravedad específica del gas - (adim) {Int64, Float64}         \n
\t Psc -> Presión pseudocrítica - (psia) {Float64}                         \n
\t Tsc -> Temperatura pseudocrítica - (°F) {Float64}                       \n
"""
function PTsc_Sutton_Ge(Ge_gas::Union{Int64, Float64})::Tuple{Float64, Float64}
...
\t Tsc -> Temperatura pseudocrítica - (°F) {Float64}   \n
"""
function PTsc_Kay(Pc::Vector{<:Real}, Tc::Vector{<:Real}, fm::Vector{<:Real})::Tuple{Float64, Float64}
    Psc, Tsc = 0, 0
...
\t Tsc -> Temperatura pseudocrítica - (°F) {Float64}   \n
"""
function PTsc_Stewart_Burkhardt_Voo(Pc::Vector{<:Real}, Tc::Vector{<:Real}, fm::Vector{<:Real})::Tuple{Float64, Float64}
    J1, J2 , K = 0, 0, 0
    for i in eachindex(fm)
        J1 += fm[i]*(Tc[i]/Pc[i])
...
"""
function PTsc_Brown_Katz_Oberfell_Alden(fm_nhc::Vector{<:Real}, pm_m::Union{Int64,Float64}, kind::String="ng")::Tuple{Float64, Float64}
    Gem = pm_m/28.96
...
\t Tsc_M -> Temperatura pseudocrítica de la mezcla - (°F) {Float64}            \n
"""
function PTsc_Sutton(fm_nhc::Vector{<:Real}, pm_m::Float64)::Tuple{Float64, Float64}
    Gem = pm_m/28.96
...
"""
function PTsr_Wichert_Aziz(Pc::Vector{<:Real},Tc::Vector{<:Real},fm::Vector{<:Real},fm_nhc::Vector{<:Real}, Py::Union{Int64,Float64},Ty::Union{Int64,Float64})::Tuple{Float64,Float64}
...
"""
function PTsc_Wichert_Aziz(Pc::Vector{<:Real},Tc::Vector{<:Real},fm::Vector{<:Real},fm_nhc::Vector{<:Real})::Tuple{Float64,Float64}
    # Factor de corrección epsilon
    eps = 120 * ((fm_nhc[1] + fm_nhc[2])^0.9 - (fm_nhc[1] + fm_nhc[2])^1.6) + 15 * (fm_nhc[2]^0.5 - fm_nhc[2]^4)
...
#---------------------------------------------------------------------------------------------------------------------------------
function PTsc_Wichert_Aziz(Pc::Vector{<:Real},Tc::Vector{<:Real},fm::Vector{<:Real},fm_co2::Union{Int64, Float64}, fm_h2s::Union{Int64, Float64})::Tuple{Float64,Float64}
    # Factor de corrección epsilon
...
#---------------------------------------------------------------------------------------------------------------------------------
function PTsc_n2_h2o(Psci::Union{Int64, Float64},Tsci::Union{Int64, Float64},fm_nhc::Vector{<:Real})::Tuple{Float64,Float64}
